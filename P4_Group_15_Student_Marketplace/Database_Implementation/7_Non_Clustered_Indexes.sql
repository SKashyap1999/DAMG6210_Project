...
/*Discount Percentage on each ITEM */

-- Create UDF to calculate discount percentage
...
SELECT 
    ...
    mr.Message_Report_ID as msg_report_id, 
    m.Sender_Student_ID as Sender, 
    m.Receiver_Student_ID as Receiver,*/
    i.Item_ID as item_id,  -- Corrected alias
...
CREATE VIEW recently_posted_items AS
SELECT
    l.Listing_ID,
    l.Date_Posted,
    i.Item_Name,
...
/*Items by Seller*/
CREATE VIEW items_by_seller AS
SELECT
...
CREATE VIEW avg_item_price_by_category AS
SELECT
    c.Category_Name,
